...
open class SupportRequestRepository {
    companion object { private val tpl = AppCrudRepoTemplate(SupportRequest::class) }

    open fun save(mc: ModuleCtx, entity: SupportRequest) = tpl.save(mc, entity)

    /** owner-scoped 详情：仅返回 appId + customerId 名下的记录。 */
    fun findByIdOwned(mc: ModuleCtx, appId: UUID, customerId: UUID, id: UUID): SupportRequest? =
    ...
    fun findMineByCursor(mc: ModuleCtx, appId: UUID, customerId: UUID, limit: Int, cursor: UUID?): Page<SupportRequest> {
