...
class AiFacade(
    ...
    fun findById(opCtx: OperationContext, id: UUID): ScanRecord? = scanHandler.findById(svcCtxFactory.forApp(opCtx), id)
    fun findByCursor(opCtx: OperationContext, cursor: String?, limit: Int?): Page<ScanRecord> =
        scanHandler.findByCursorFiltered(svcCtxFactory.forApp(opCtx), cursor, limit, null)
    fun findByCursorFiltered(opCtx: OperationContext, cursor: String?, limit: Int?, collected: Boolean?): Page<ScanRecord> =
        scanHandler.findByCursorFiltered(svcCtxFactory.forApp(opCtx), cursor, limit, collected)
    fun findByFilter(opCtx: OperationContext, filter: FilterGroup?, cursor: String?, limit: Int?): Page<ScanRecord> =
        scanHandler.findByFilter(svcCtxFactory.forApp(opCtx), filter, cursor, limit)
    ...
    fun newScan(opCtx: OperationContext, input: com.ifmix.api.core.generated.types.NewScanInput): ScanRecord {
        ...
        // Step 2: DB 写入（有事务）
        return tx.withTx(svcCtxFactory.forApp(opCtx)) { sc -> scanHandler.saveNewScan(sc, scanId, input) }
    }
    ...
    fun deleteScan(opCtx: OperationContext, id: UUID): Boolean =
        tx.withTx(svcCtxFactory.forApp(opCtx)) { sc -> scanHandler.deleteScan(sc, id) }
    fun presignedUploadUrl(opCtx: OperationContext, objectKey: String, contentType: String, duration: Duration): String =
        scanHandler.presignedUploadUrl(svcCtxFactory.forApp(opCtx), objectKey, contentType, duration)
    fun presignedDownloadUrl(opCtx: OperationContext, objectKey: String, duration: Duration): String =
        scanHandler.presignedDownloadUrl(svcCtxFactory.forApp(opCtx), objectKey, duration)
    fun getPublicUrl(opCtx: OperationContext, objectKey: String): String =
        scanHandler.getPublicUrl(svcCtxFactory.forApp(opCtx), objectKey)
}
