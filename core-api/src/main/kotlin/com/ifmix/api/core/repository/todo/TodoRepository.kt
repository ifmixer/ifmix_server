...
class TodoRepository(sql: KSqlClient) : BaseAppCrudRepository<Todo>(sql, Todo::class) {
    ...
    /** 按租户 + id 取单条视图。 */
    fun findViewById(appId: UUID, id: UUID): TodoView? =
    ...
    fun findViewByCursorForApp(appId: UUID, input: CursorQueryInput): Page<TodoView> {
    ...
    fun create(appId: UUID, input: TodoCreateInput): Todo {
        val now = Instant.now()
        val entity = Todo {
            id = UuidV7.generate()
            this.appId = appId
    ...
    fun update(appId: UUID, input: TodoUpdateInput): Todo? {
    ...
    fun deleteForApp(appId: UUID, id: UUID): Boolean {
