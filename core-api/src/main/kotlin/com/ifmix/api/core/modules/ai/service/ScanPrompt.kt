...
/**
 * V4 Production Prompt for AI antique scanning.
 *
 * Architecture:
 * - [SYSTEM_BODY]: Static reasoning rules, output schema, safety — never changes per-request.
 * - [systemPrompt]: "RESPOND IN: xx" + SYSTEM_BODY → complete system message.
 * - [userPrompt]: RUNTIME_CONTEXT + TASK → short dynamic user message. Log this for debugging.
 */
object ScanPrompt {

    // @formatter:off
    /**
     * Static body of the system prompt — all reasoning rules, output schema, safety.
     * Language directive is prepended dynamically by [systemPrompt].
     */
    private const val SYSTEM_BODY = """You are a professional visual analysis engine for antiques, vintage objects, collectibles, decorative arts, coins, jewelry, ceramics, glass, furniture, artworks, watches, toys, memorabilia, books, documents, and related objects.
...
The user message will supply runtime context values:

* `current_date`: ISO-8601 date for age classification thresholds.
* `response_language`: BCP-47 tag for user-facing text output.
* `market_region`: ISO 3166-1 alpha-2 code indicating the user's market for valuation context.
* `valuation_currency`: ISO 4217 code for price fields.

If `response_language` is missing, use English.
If `valuation_currency` is missing, use USD.
If `market_region` is missing, do not assume any specific market.

All user-facing descriptive and explanatory text must use `response_language`.

Always keep these in English regardless of response_language:
...
Never translate, normalize, or rewrite verbatim visual evidence:

* `visible_text.text` must preserve the exact characters seen in the image.
* `marks.text` must preserve the exact characters seen in the image.

User location (market_region) affects valuation market context only. Never use it as evidence for object origin, age, authenticity, rarity, or identification.
...
Use these general rules relative to `current_date` provided in RUNTIME_CONTEXT:
...
Use `valuation_currency` from RUNTIME_CONTEXT when supplied.

Otherwise use `USD`.

## valuation.market_region

Use supplied `market_region` from RUNTIME_CONTEXT.
...
24. `market_region` was never used as evidence for origin, age, authenticity, rarity, or identification."""
    ...
    /**
     * Build the complete system prompt with language directive prepended.
     *
     * Structure: "RESPOND IN: xx" (dynamic) + static reasoning rules body.
     *
     * @param input scan input (uses lang to determine response language)
     */
    fun systemPrompt(input: ScanInput): String {
        val resolvedLang = input.lang?.takeIf { it.isNotBlank() } ?: "en-US"
        return "RESPOND IN: $resolvedLang\n\n${SYSTEM_BODY.trimIndent()}"
    }

    /**
     * Build the dynamic user prompt — runtime context + task.
     *
     * This is short and per-request varying. Log this for debugging.
     *
     * @param input scan input containing images, lang, country, currency, date
     */
    fun userPrompt(input: ScanInput): String {
        ...
        val task = if (imageCount == 1) {
            "Analyze the attached image of the primary object."
        } else {
        ...
        return buildString {
            ...
            appendLine("- current_date: ${input.date}")
            appendLine("- response_language: $resolvedLang")
            if (!input.country.isNullOrBlank()) {
                appendLine("- market_region: ${input.country}")
            }
            appendLine("- valuation_currency: $resolvedCurrency")
            appendLine()
            appendLine("TASK:")
            appendLine(task)
            append("Perform the antique/collectible visual assessment according to the system instructions.")
        }
    }
}
