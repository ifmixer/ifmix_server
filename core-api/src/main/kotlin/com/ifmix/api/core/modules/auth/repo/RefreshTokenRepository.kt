...
import com.ifmix.api.core.entity.auth.actorId
import com.ifmix.api.core.entity.auth.actorType
...
class RefreshTokenRepository {
    companion object { private val tpl = AppCrudRepoTemplate(RefreshToken::class) }

    fun findValidByHash(mc: ModuleCtx, appId: UUID, tokenHash: String): RefreshToken? {
        val now = Instant.now()
     ...
     */
    fun hasValidToken(mc: ModuleCtx, appId: UUID, actorId: UUID, actorType: String): Boolean {
        val now = Instant.now()
        return mc.sql.createQuery(RefreshToken::class) {
            where(table.appId eq appId)
    ...
    fun revokeAllByActor(mc: ModuleCtx, appId: UUID, actorId: UUID, actorType: String): Int {
        val now = Instant.now()
        return mc.sql.createUpdate(RefreshToken::class) {
            where(table.appId eq appId)
            where(table.actorId eq actorId)
