...
class BaseRepositoryTest {
    ...
    fun softDeleteHidesFromFindAndList() {
        ...
        assertThat(repo.findByCursor(ctx, input = CursorQueryInput(limit = 10)).items).isEmpty()
    }
    ...
    fun findByCursorPaginatesDescendingById() {
        ...
        val p1 = repo.findByCursor(ctx, input = CursorQueryInput(order = CursorQueryInput.Order.DESC, limit = 2))
        ...
        val p2 = repo.findByCursor(ctx, input = CursorQueryInput(cursor = p1.nextCursor, order = CursorQueryInput.Order.DESC, limit = 2))
