...
IdpIdentity (全局, 跨 app)              — IDP 下的第三方身份，按 (idpId, providerSubjectId) 唯一
AuthIdentity (app 级)                   — 账号中枢：password + 账号权威资料（姓名/邮箱/手机/metadata）
AuthIdentityIdpRelation (app 级)        — AuthIdentity ↔ IdpIdentity 的 M:N 绑定（软删）
Customer (app 级)                       — App 内 C 端用户：匿名/合并语义 + authIdentityId
RefreshToken (app 级, auth_refreshtoken) — Refresh Token（actorType + actorId，主体无关）
AppToIdpRelation (app 级)               — App 启用了哪些 IDP
```

### 为什么是这套分层

- **`IdpIdentity` 全局、跨 app**：同一第三方 sub 在不同 app 复用同一行。
- **`AuthIdentity` app 级**：账号中枢，持密码与用户权威资料。与 `Customer` 是 **1:N**（customer 逻辑注销后可新建、复用同一 auth_identity）。
- **M:N 用关系表**：`IdpIdentity ↔ AuthIdentity` 是多对多，用关系表 `auth_identity_to_idpidentity_relation` 表达（双向普通 B-tree 索引点查，不用数组列/JSONB）。
- **`Customer` 精简**：只保留 app 级身份与合并语义（`anonymous / mergedTo / mergedToAt / authIdentityId`），资料归 `AuthIdentity`。

### AuthIdentity ↔ Actor（Customer / Manager）

一个 `AuthIdentity`（app 级账号）映射一到两类**主体（actor）**：

- **Customer**（C 端）：`customer.authIdentityId` → `auth_identity.id`。
- **Manager**（B 端，规划中）：同 app 下同一 `AuthIdentity` 也可被 manager 主体引用。

即：账号（`AuthIdentity`）是"人"，`Customer`/`Manager` 是这个人在同一 app 内的不同**角色身份**。`RefreshToken` 因此主体无关，用 `actorType`（10=customer / 20=manager）+ `actorId` 关联，而非直接绑 customer。

### IdpIdentity ↔ AuthIdentity 的双向多对多

关系是真正的 **M:N**，两个方向都成立，正是用关系表而非内嵌数组的原因：

- **一个 IdpIdentity → 多个 AuthIdentity（跨 app）**：同一个 Google 身份（`IdpIdentity`，全局唯一）被多个 app 各自的 `AuthIdentity` 绑定——每个 app 一条 relation。
- **一个 AuthIdentity → 多个 IdpIdentity（同 app 多 provider）**：同一 app 内一个账号可同时绑定 Google + Apple 等多个第三方身份——每个 provider 一条 relation。

关系表按 app 隔离（`app_id` 列）：反查方向 `(app_id, idp_identity_id)` 未删记录唯一（一个身份在一个 app 内只归属一个账号），正查方向 `(app_id, auth_identity_id)` 可多行（一个账号名下多个身份）。
...
| app 级用户身份、匿名、合并 | `Customer` | 匿名先行、登录转正、跨设备合并 |
...
5. 判定 existing：
   idpIdentity → relation(app 级, findByAppAndIdpIdentity) → auth_identity
...
| # | 条件 | 动作 |
|---|------|------|
| ① PromoteOrCreate | `existing == null`（此 app 下该身份无账号） | `target = cur ?: 新建 customer`；建 `auth_identity` + 关系；`customer.authIdentityId = authId`；`cur != null` 时转正 |
...
- token claim：`sub=actorId, act=actorType(Int), aud=appId, ano=anonymous`（见 `AuthJwtService`）
