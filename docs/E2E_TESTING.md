...
**推荐: Spring Boot `@SpringBootTest` + Testcontainers PostgreSQL + `TestRestClient`**

理由：
1. 与项目栈一致（Kotlin + JUnit 5），不引入额外语言
2. Testcontainers 自动管理 PostgreSQL + Redis 生命周期
3. Flyway 自动执行全部 migration（包含 V8 表前缀）
...
├───────────────────────────────────────────────────────────────┤
│                                                                │
│  TestFixtures         →  Seed data (AppConfig, Tenant, etc)    │
│  TestRestClient       →  HTTP calls (real HTTP, real server)   │
...
```
core-api/src/test/kotlin/com/ifmix/api/core/
├── e2e/                          # E2E 测试包
│   ├── support/
...
```bash
# 本地运行全部 E2E
./gradlew :core-api:test --tests "com.ifmix.api.core.e2e.*"
...
# 只跑某个模块
./gradlew :core-api:test --tests "com.ifmix.api.core.e2e.AuthE2eTest"
```
