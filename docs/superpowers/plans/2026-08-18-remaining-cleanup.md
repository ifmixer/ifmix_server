...
- DGS typeMapping：`Todo`/`TodoItem`/`ScanCollection` 使用 domain entity

### TypeMapping 完善 + Fetcher 清理（提交 `9f72978`）
- `ScanRecord` → domain entity（field alias `images` = `imageKeys`）
- `ImageRef` → domain entity
- `ScanCollection` → domain entity
- `ScanFetcher` — 去掉 `toDgs()` 转换，直接返回 domain entity
- `CollectionFetcher` — 去掉 `Dgs*` alias，直接使用 domain entity
- `ScanCollectionFacadeService.findItemsByCursorPage` — 用 stub ScanRecord 填充 scanRecord 字段（暂未实现 DataFetcher）
- `ScanCollectionItem` 保持 generated type（关系表，字段与 domain entity 不一致）

### TypeMapping 策略总结
| 类型 | 处理 |
|------|------|
| Todo / TodoItem | domain entity ✅ |
| ScanRecord | domain entity ✅ |
| ScanCollection | domain entity ✅ |
| ImageRef | domain entity ✅ |
